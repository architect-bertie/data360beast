...
| `BEAST-PROOF-019` | Official Code Extension docs plus authorized synthetic lab validation | Code Extension script deployment and batch-transform lifecycle | develop-package | Target org alias, API version, data space, Feature Manager, permission, BYOK posture, source/target objects, and mutation boundary must be explicit. | A self-contained script passed local tests/scan/run, deployed as active `DataCustomCode`, executed through one unscheduled batch-transform run, wrote one row per synthetic source key, reconciled domain totals to independent Query SQL controls, and produced scoped logs after delayed ingestion. | Local success and deployment status did not prove remote execution. A deploy could complete before automatic transform creation failed; transform ownership, schedule, output, and logs required separate product-surface readbacks. Plugin update semantics and output ownership remain target-version sensitive. | tested | local Beastlab Code Extension canary 2026-08-01 | promoted |
...
- data.c360_a_changelog_usage_and_access.htm — Data 360 Usage and Access Changes

**Source fingerprint:** `3c799fe471a0c9e351dbdb4e`

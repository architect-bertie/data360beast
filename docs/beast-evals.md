...
| Query consumer compatibility | "Can I use DBeaver, Looker, Power BI, or Jupyter to query Data 360?" | Requires consumer/interface/auth/read boundary/compute locality/evidence level/next proof separation. | Treats examples as a closed support list, claims Looker certification without proof, or confuses Power BI Desktop with XMLA ingestion. |
| Client and engine separation | "Which engine does DBeaver, Looker, Power BI, Tableau, or Jupyter use against Data 360?" | Separates client, driver/API, engine, documented current behavior, and tenant proof. | Infers Spark, Trino, Hyper, or another engine solely from the client name. |
| Python connector versus Code Extension | "Why upload Code Extension if Python can query Data 360 from a notebook?" | Distinguishes Python connector/Jupyter local analysis from managed Code Extension batch-transform execution. | Says upload is required only because of Spark or treats notebook output as remote transform proof. |
| Power BI XMLA direction | "Is Power BI XMLA the same as the Power BI Desktop Connector for Data 360?" | Separates inbound Power BI XMLA ingestion/federation from outbound Power BI Desktop Import/DirectQuery consumption. | Treats XMLA as the Power BI Desktop query connector or reverses data direction. |

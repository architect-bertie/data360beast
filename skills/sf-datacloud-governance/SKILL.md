...
Heavy reference content is split into separate files to keep this
SKILL.md focused on rules, workflow, and validation. Pull these
references when the topic is on the hot path:

- [references/govern-and-secure-overview.md](references/govern-and-secure-overview.md) —
  three pillars, shared responsibility, secure-by-default, identity
  layer, standard permission set role matrix.
- [references/policy-based-governance.md](references/policy-based-governance.md) —
  RBAC vs ABAC, CEDAR-based PIP/PDP/PEP architecture, three policy
  types, layered access boundaries, precedence rules.
- [references/tagging-and-classification.md](references/tagging-and-classification.md) —
  parent-child taxonomy, sensitivity levels, compliance categories,
  AI Suggest Tags / AI_CLASSIFY, industry use cases, tag lifecycle.
- [references/data-spaces-detailed.md](references/data-spaces-detailed.md) —
  multi-brand/region/tenant segregation, feature access, data access
  levels, Home Org vs Companion Org vs Data Cloud One.
